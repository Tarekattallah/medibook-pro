...
const DOCTORS = [
  { name:'Dr. Sarah Johnson',   specialty:'Cardiology',    location:'Cairo, Nasr City',      price:350, yearsExperience:12, languages:['English','Arabic'], bio:'Board-certified cardiologist with 12 years specializing in preventive cardiology and heart failure management.', rating:4.9, reviewCount:0, isVerified:true },
  { name:'Dr. Michael Chen',    specialty:'Neurology',     location:'Cairo, Heliopolis',     price:300, yearsExperience:8,  languages:['English','Arabic'], bio:'Expert neurologist focusing on headache disorders, epilepsy, and movement disorders.', rating:4.8, reviewCount:0, isVerified:true },
  ...
  { name:'Dr. James Wilson',    specialty:'Orthopedics',   location:'Cairo, Maadi',          price:400, yearsExperience:15, languages:['English','Arabic'], bio:'Orthopedic surgeon specializing in sports injuries and joint replacement surgery.', rating:4.9, reviewCount:0, isVerified:true },
  ...
  { name:'Dr. Robert Kim',      specialty:'Psychiatry',    location:'Cairo, Zamalek',        price:280, yearsExperience:9,  languages:['English','Arabic'], bio:'Licensed psychiatrist specializing in mood disorders, anxiety, and CBT.', rating:4.8, reviewCount:0, isVerified:true },
...
async function seed() {
  try {
    ...
    const admin = await User.create({
      name:'Admin User', email:'admin@demo.com', password:hashed, role:'admin'
    })

    const patient = await User.create({
      name:'Ahmed Mohamed', email:'patient@demo.com', password:hashed,
      role:'patient', phone:'+20 10 1234 5678',
      dateOfBirth: new Date('1990-05-15'), bloodType:'A+'
    })

    const demoDoctor = await User.create({
      name:'Dr. Sarah Johnson', email:'doctor@demo.com', password:hashed,
      role:'doctor', specialty:'Cardiology', location:'Cairo, Nasr City',
      price:350, yearsExperience:12, languages:['English','Arabic'],
      bio:'Board-certified cardiologist with 12 years experience in preventive cardiology.',
    ...
    for (const doc of DOCTORS) {
      ...
      const d = await User.create({ ...doc, email, password:hashed, role:'doctor', availability:avail })
      doctors.push(d)
    }
    console.log(`✅ ${doctors.length + 1} doctors created`)

    // --- Seed sample appointments for demo patient ---
    ...
    // --- Seed reviews from completed appointments ---
    await Promise.all([
      Review.create({ patient:patient._id, doctor:demoDoctor._id, appointment:apt1._id, rating:5, comment:'Dr. Johnson was thorough and very professional. Highly recommend!' }),
      Review.create({ patient:patient._id, doctor:doctors[1]._id,  appointment:apt2._id, rating:5, comment:'Excellent consultation. Very knowledgeable and patient.' }),
    ])
    ...
    // --- Seed notifications for demo patient ---
    await Promise.all([
      ...
      Notification.create({ user:patient._id, type:'system',      title:'Profile Incomplete',     body:'Complete your profile to get personalized doctor recommendations.', link:'/settings', read:true }),
